...
@section('meta')
    @include('partials.meta', ['pageKey' => 'career'])
@endsection

@section('content')
    <!-- ===== HERO ===== -->
    <section
        class="fixed hero-fixed  top-0 left-0 w-full z-0 overflow-hidden
                h-[600px] md:h-[700px] lg:h-[900px]">
        <!-- Background Image -->
        <img src="{{ asset($career->img_path ?? '') }}" alt="interior" class="absolute inset-0 w-full h-full object-cover">
            ...
            <div class="relative z-10 container mx-auto  ">
                ...
                <div class="flex flex-col md:flex-row gap-10 items-start">
                        ...
                        @if ($careerOverview?->img_path)
                            <div class="relative z-10">
                                <img src="{{ asset($careerOverview->img_path) }}" alt="Career"
                                    class="w-full object-cover shadow-sm" style="height: 450px;">
                            </div>
                        @endif
                    ...
                    <div class="w-full md:w-8/12">
                        @if ($rightImg)
                            <img src="{{ asset($rightImg) }}" alt="Office" class="w-full object-cover scroll-move"
                                data-axis="Y" style="height: 450px;">
                        @endif
                    </div>
                </div>
        ...
        <div class="job-position">
            <div class="absolute inset-0 pointer-events-none z-0">
                <img src="{{ asset('images/career-bg.webp') }}" alt="career bg"
                    class="w-full h-full object-cover opacity-10" onerror="this.style.display='none';">
            </div>
            ...
            <div class="absolute bottom-0 right-0 pointer-events-none z-0">
                <img src="{{ asset('assets/images/reviewstonebg.webp') }}" alt="review stone"
                    style="width:120px; opacity:0.85;" onerror="this.style.display='none';">
            </div>

            <div class="relative z-10 container mx-auto px-6 lg:px-14">

                <!-- Heading -->
                <h2 class="font-light text-gray-900 mb-12" style="font-size:clamp(28px,4.5vw,64px); font-weight:300;">
                    Available job positions
                </h2>
    ...
    <section id="apply-job-section" class="relative w-full overflow-hidden py-20" style="background:#1B281F;">
        ...
        <div class="absolute inset-0 pointer-events-none z-0">
            <img src="{{ asset('images/form-bg.webp') }}" alt="form bg" class="w-full h-full object-cover opacity-20"
                onerror="this.style.display='none';">
        </div>
        ...
        <div class="relative z-10 container mx-auto px-6 lg:px-14">
            ...
            <form action="{{ route('job.apply') }}" method="POST" enctype="multipart/form-data" class="space-y-10">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                    ...
                    <div>
                        <div
                            style="border-bottom:1px solid {{ $errors->has('name') ? '#ef4444' : 'rgba(255,255,255,0.25)' }};">
                    ...
                    <div>
                        <div
                            style="border-bottom:1px solid {{ $errors->has('phone') ? '#ef4444' : 'rgba(255,255,255,0.25)' }};">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-10">
                    ...
                    <div>
                        <div
                            style="border-bottom:1px solid {{ $errors->has('email') ? '#ef4444' : 'rgba(255,255,255,0.25)' }};">
                    ...
                    <div>
                        <div
                            style="border-bottom:1px solid {{ $errors->has('subject') ? '#ef4444' : 'rgba(255,255,255,0.25)' }};">
                ...
                <div>
                    <div
                        style="border-bottom:1px solid {{ $errors->has('content_id') ? '#ef4444' : 'rgba(255,255,255,0.25)' }};">
                        ...
                        <select id="job_position" name="content_id"
                            ...
                            @foreach ($jobList as $job)
                                <option value="{{ $job->id }}" class="bg-[#1B281F] text-white"
                                    {{ old('content_id') == $job->id ? 'selected' : '' }}>
                                    {{ $job->title }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <label for="resumeUpload" class="flex items-center gap-3 cursor-pointer w-fit">
                        <div
                            class="w-12 h-12 rounded-full border {{ $errors->has('resume') ? 'border-red-500' : 'border-white/40' }} flex items-center justify-center transition-all duration-300 hover:border-white hover:bg-white/10">
                        ...
                        <span id="fileLabel" class="text-white text-sm font-light opacity-70">Attach Your
                            Resume*</span>
                    </label>
